...
@Component({
    selector: 'jhi-message-freelancr',
    templateUrl: './message-freelancr.component.html'
})
export class MessageFreelancrComponent implements OnInit, OnDestroy {
...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.messageService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<MessageFreelancr[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/cms/message-freelancr'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/cms/message-freelancr', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: MessageFreelancr) {
        return item.id;
    }
