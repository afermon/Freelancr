...
@Component({
    selector: 'jhi-position-freelancr',
    templateUrl: './position-freelancr.component.html'
})
export class PositionFreelancrComponent implements OnInit, OnDestroy {
...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.positionService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<PositionFreelancr[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/cms/position-freelancr'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/cms/position-freelancr', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PositionFreelancr) {
        return item.id;
    }
