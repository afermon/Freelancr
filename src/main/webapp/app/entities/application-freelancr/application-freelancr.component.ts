...
@Component({
    selector: 'jhi-application-freelancr',
    templateUrl: './application-freelancr.component.html'
})
export class ApplicationFreelancrComponent implements OnInit, OnDestroy {
...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.applicationService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<ApplicationFreelancr[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/cms/application-freelancr'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/cms/application-freelancr', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ApplicationFreelancr) {
        return item.id;
    }
